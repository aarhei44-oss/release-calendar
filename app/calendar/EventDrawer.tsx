...
export function EventDrawer({ eventId, onClose }: Props) {
  const [fetched, setFetched] = useState<{ eventId: string; detail: EventDetail } | null>(null);
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      >
        ...
        {!loading && !detail && (
          <p className="mt-4 text-sm text-gray-500">This event could not be found.</p>
        )}
        ...
        {!loading && detail && (
          <div className="mt-2 flex flex-col gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <span className={`rounded px-2 py-0.5 text-xs font-medium ${statusBadgeClass(detail.status)}`}>
                {detail.status}
              </span>
            ...
            <div>
              ...
              <p className="text-sm text-gray-900">
                ...
                <span className="text-gray-500">({detail.dateType.toLowerCase()})</span>
              </p>
              {detail.manualNotes && (
                <p className="mt-1 text-sm text-gray-600">{detail.manualNotes}</p>
              )}
            </div>
            ...
            <div>
              ...
              {detail.sourceClaims.length === 0 ? (
                <p className="text-sm text-gray-500">No source claims recorded yet.</p>
              ) : (
                <ul className="mt-2 flex flex-col gap-2">
                  {detail.sourceClaims.map((claim) => (
                    <li key={claim.id} className="rounded-md border border-gray-200 p-2 text-sm">
                      <div className="flex items-center justify-between">
                        <span className="font-medium">{claim.tier}</span>
                        <span className="text-xs text-gray-500">{claim.disposition}</span>
                      </div>
                      <a
                        href={claim.url}
                        target="_blank"
